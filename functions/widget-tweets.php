...
class mts_widget_recent_tweets extends WP_Widget {
	public function __construct() {
		parent::__construct(
			...
			__( 'MTS Latest Tweets', MTS_THEME_TEXTDOMAIN ),// Name
			array( 'description' => __( 'A widget for showing latest tweets in sidebar', MTS_THEME_TEXTDOMAIN ), ) // Args
		);
	}
	public function widget($args, $instance) {
		...
		if ( empty( $tweets ) ) { _e( 'Couldn\'t retrieve tweets! Wrong username?', MTS_THEME_TEXTDOMAIN ); echo $after_widget; return; }
		if(!empty($tweets->errors)){
			if($tweets->errors[0]->message == 'Invalid or expired token'){
				echo '<strong>'.$tweets->errors[0]->message.'!</strong><br />'. esc_html( 'You\'ll need to regenerate it <a href="https://dev.twitter.com/apps" target="_blank">here</a>!', MTS_THEME_TEXTDOMAIN ) . $after_widget;
	...
	echo '<p><label>Title:</label><input type="text" name="'.$this->get_field_name( 'title' ).'" id="'.$this->get_field_id( 'title' ).'" value="'.esc_attr($instance['title']).'" class="widefat" /></p><p><label>'.__('API Key', MTS_THEME_TEXTDOMAIN ).':</label><input type="text" name="'.$this->get_field_name( 'consumerkey' ).'" id="'.$this->get_field_id( 'consumerkey' ).'" value="'.esc_attr($instance['consumerkey']).'" class="widefat" /></p><p><label>'.__('API Secret', MTS_THEME_TEXTDOMAIN ).':</label><input type="text" name="'.$this->get_field_name( 'consumersecret' ).'" id="'.$this->get_field_id( 'consumersecret' ).'" value="'.esc_attr($instance['consumersecret']).'" class="widefat" /></p><p><label>'.__('Access Token', MTS_THEME_TEXTDOMAIN ).':</label><input type="text" name="'.$this->get_field_name( 'accesstoken' ).'" id="'.$this->get_field_id( 'accesstoken' ).'" value="'.esc_attr($instance['accesstoken']).'" class="widefat" /></p>
	<p><label>'.__('Access Token Secret', MTS_THEME_TEXTDOMAIN ).':</label><input type="text" name="'.$this->get_field_name( 'accesstokensecret' ).'" id="'.$this->get_field_id( 'accesstokensecret' ).'" value="'.esc_attr($instance['accesstokensecret']).'" class="widefat" /></p><p><label>'.__('Cache Tweets in every', MTS_THEME_TEXTDOMAIN ).':</label><input type="text" name="'.$this->get_field_name( 'cachetime' ).'" id="'.$this->get_field_id( 'cachetime' ).'" value="'.esc_attr($instance['cachetime']).'" class="small-text" /> hours</p><p><label>'.__('Twitter Username', MTS_THEME_TEXTDOMAIN ).'</label>
	<input type="text" name="'.$this->get_field_name( 'username' ).'" id="'.$this->get_field_id( 'username' ).'" value="'.esc_attr($instance['username']).'" class="widefat" /></p><p><label>'.__('Number of tweets (max 20)', MTS_THEME_TEXTDOMAIN ).'</label><select type="text" name="'.$this->get_field_name( 'tweetstoshow' ).'" id="'.$this->get_field_id( 'tweetstoshow' ).'">';
	...
	public function relative_time($a) {			
		...
		$week = $day * 7;				
		if(is_numeric($d) && $d > 0) {				
			if($d < 3) return __( "right now", MTS_THEME_TEXTDOMAIN ); //if less then 3 seconds
			if($d < $minute) return sprintf( __("%s seconds ago", MTS_THEME_TEXTDOMAIN ), floor($d) ); //if less then minute
			if($d < $minute * 2) return __( "about 1 minute ago", MTS_THEME_TEXTDOMAIN ); //if less then 2 minutes
			if($d < $hour) return sprintf( __( "%s minutes ago", MTS_THEME_TEXTDOMAIN ), floor( $d / $minute ) ); //if less then hour
			if($d < $hour * 2) return __( "about 1 hour ago", MTS_THEME_TEXTDOMAIN ); //if less then 2 hours
			if($d < $day) return sprintf( __("%s hours ago", MTS_THEME_TEXTDOMAIN ), floor( $d / $hour ) ); //if less then day
			if($d > $day && $d < $day * 2) return __( "yesterday", MTS_THEME_TEXTDOMAIN ); //if more then day, but less then 2 days
			if($d < $day * 365) return sprintf(  __( "%s days ago", MTS_THEME_TEXTDOMAIN ), floor( $d / $day ) ); //if less then year
			return __( "over a year ago", MTS_THEME_TEXTDOMAIN ); //else return more than a year
		}
	}
}	
// register	widget
