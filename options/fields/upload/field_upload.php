...
class NHP_Options_upload extends NHP_Options{	
	...
	*/
	function render(){
		...
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'">'.__('Browse', MTS_THEME_TEXTDOMAIN ).'</a>';
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'">'.__('Remove Upload', MTS_THEME_TEXTDOMAIN ).'</a>';
	...
	*/
	function enqueue(){
		...
		wp_localize_script('nhp-opts-field-upload-js', 'nhp_upload', array(
			...
			'title' => __( 'Select Image', MTS_THEME_TEXTDOMAIN ),
			'buttonText' => __( 'Select Image', MTS_THEME_TEXTDOMAIN )
		));
